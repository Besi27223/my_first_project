...
## גרסה שלישית: מסך "העלאת קבלה" בלבד

לפריסה מצומצמת שמיועדת רק לתיעוד קבלות (למשל למסירה למישהו שרק סורק/ת
קבלות, בלי גישה לשאר המסכים) — הגדירו `NEXT_PUBLIC_UPLOAD_ONLY=true`
בסביבת ה-Vercel של אותה פריסה (פרויקט Vercel נפרד, מחובר לאותו repo/ענף,
כמו מצב הדמו). האפליקציה תציג רק את מסך "חשבונית חדשה", ללא ניווט תחתון;
ניסיון גישה ישירה לשאר המסכים (`/list`, `/calc`, `/reports`) יחזיר
אוטומטית למסך ההעלאה. משתמשת באותו Supabase/Dropbox אמיתיים של הפריסה
המלאה — אינה מצב דמו בפני עצמו (אפשר לשלב את שני המשתנים יחד אם רוצים
גרסת דמו-להעלאה-בלבד).
